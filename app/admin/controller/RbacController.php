...
class RbacController extends AdminBaseController
{
    ...
    public function roleAddPost()
    {
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                if ($result) {
                    RecordLogModel::getSelfInstance()->add(Config::LOG_TYPE_3, $result);//写日志
        ...
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                if (Db::name('role')->update($data) !== false) {
                    RecordLogModel::getSelfInstance()->add(Config::LOG_TYPE_4, $id);//写日志
    ...
    public function roleDelete()
    {
        ...
        if ($count > 0) {
            ...
            if (!empty($status)) {
                RecordLogModel::getSelfInstance()->add(Config::LOG_TYPE_7, $id);//写日志
    ...
    public function authorizePost()
    {
        if ($this->request->isPost()) {
            ...
            if (is_array($this->request->param('menuId/a')) && count($this->request->param('menuId/a')) > 0) {
                ...
                RecordLogModel::getSelfInstance()->add(Config::LOG_TYPE_5, $roleId);//写日志
                $this->success("授权成功！");
            } else {
                //当没有数据时，清除当前角色授权
