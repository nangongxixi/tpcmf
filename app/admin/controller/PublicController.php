...
class PublicController extends AdminBaseController
{
    ...
    public function doLogin()
    {
        ...
        if (!empty($result) && $result['user_type'] == 1) {
            if (cmf_compare_password($pass, $result['user_pass'])) {
                ...
                $result['login_token'] = uniqid();
                session('login_token', $result['login_token']);

                Db::name('user')->update($result);
                cookie("admin_username", $name, 3600 * 24 * 30);
                session("__LOGIN_BY_CMF_ADMIN_PW__", null);
                RecordLogModel::getSelfInstance()->add(Config::LOG_TYPE_1, cmf_get_current_admin_id());//写日志
    ...
    {
        RecordLogModel::getSelfInstance()->add(Config::LOG_TYPE_2, cmf_get_current_admin_id());//写日志
