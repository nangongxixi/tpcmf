<?php /*a:2:{s:83:"D:\phpstudy_pro\WWW\tpcmf\public/themes/admin_simpleboot3/admin\customer\index.html";i:1569631099;s:76:"D:\phpstudy_pro\WWW\tpcmf\public/themes/admin_simpleboot3/public\header.html";i:1569667776;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="wrap js-check-wrap">
    <ul class="nav nav-tabs">
        <li class="active"><a href="<?php echo url('customer/index'); ?>">客户查询</a></li>
    </ul>
    <form class="well form-inline margin-top-20" method="get" action="<?php echo url('customer/index'); ?>">
        客户姓名:
        ...
        <a class="btn btn-danger" href="<?php echo url('customer/index'); ?>">清空</a>
    </form>
    <table class="table table-hover table-bordered">
        ...
        <?php if(is_array($customerList) || $customerList instanceof \think\Collection || $customerList instanceof \think\Paginator): if( count($customerList)==0 ) : echo "" ;else: foreach($customerList as $key=>$vo): ?>
            <tr>
                ...
                <td style="color:<?php echo $statusColor[$vo['user_status']]; ?>"><?php echo $statusList[$vo['user_status']]; ?></td>
                <td><?php echo isset($workerList[$vo['worker_uid']]) ? $workerList[$vo['worker_uid']] : ''; ?></td>
                <td><?php echo $vo['create_time']; ?></td>
                <td>
                    <!-- 按钮判断 -->
                    <a class="btn btn-xs btn-danger" href="<?php echo url('customer/info',array('id'=>$vo['id'])); ?>">查看</a>

                    <?php if(cmf_get_current_admin_id() == $vo['worker_uid'] || cmf_get_current_admin_id() == 1): ?>
                        <a class="btn btn-xs btn-primary" href='<?php echo url("customer/edit",array("id"=>$vo["id"])); ?>'>编辑</a>
                        <div class="btn btn-xs btn-success hljs-prompt my-worker" title="<?php echo $vo['worker_uid']; ?>" data-id="<?php echo $vo['id']; ?>">所属员工</div>
                    <?php endif; if(cmf_get_current_admin_id() == $vo['worker_uid'] || cmf_get_current_admin_id() == 1): if($vo['user_status'] == 1): ?>
                            <a class="btn btn-xs btn-danger js-ajax-dialog-btn"
                               href="<?php echo url('customer/changeStatus',array('id'=>$vo['id'], 'type'=>2)); ?>"
                               data-msg="您确定要停用此用户吗？">停用</a>
                        <?php else: ?>
                            <a class="btn btn-xs btn-success js-ajax-dialog-btn"
                               href="<?php echo url('customer/changeStatus',array('id'=>$vo['id'], 'type'=>1)); ?>"
